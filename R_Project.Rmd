...
ggplot(df.daysloaned, aes(x=length.days)) + geom_histogram() +xlim(c(0,1000))+
  xlab("Loan Duration (Days)") 
...
#with ggplot 
distrib <- enframe(rexp(my.data, rate= 0.006577415))
# base plot object
p <- ggplot(distrib, aes(x = value), freq=T)
# add command to produce histogram and select the number of bins
# overlay density curve (converted to common axis of count)
p + geom_histogram(fill="black", colour="light pink", alpha = 0.25, binwidth=0.5) +
...
quantile(lambda.hat,probs=c(.025,.975))
...
set.seed(100)
u = runif(100)
x= (log(-u+1))/-0.00660599

hist(x, probability = TRUE, main="n=100 sampled values from f(x)=lambdae^{-lambdax}")
lines(x = density(x = red.line), col = "red")
...
Determine the average length in days for a loan for each group of Carnegie Mellon-affiliated patrons, by which we mean each element of the `Patron Group` column that begins with "Carnegie Mellon". (There are five in all.) 

To be clear: you are to approach this as a "split-apply-combine" problem, as opposed to extracting information for each group separately one after the other. A few notes here that might be helpful: (1) you have to initially filter the `Patron Group` column *in the exact same manner* that you filtered the loan dates in Q4, so that each element of the filtered `Patron Group` column matches 1-to-1 with a loan duration; 

(2) after this, you filter the patron group and loan duration vectors *again*, to limit yourself to CMU-affiliated patrons; and (3) if `pg` is your filtered patron group vector, do `pg = droplevels(pg)` so that the other non-CMU-related factor levels are dropped (otherwise you'll get output for *all* patron groups, with most displaying `NA` for the average loan duration). You can apply either base `R` or `tidyverse` functions to compute your answers. Note that I personally get 184.16142 for CMU faculty: we keep items for six months on average.
```{r linewidth=80}

patron.df = cbind(dftime,newdf$`Patron Group`)
patron.df = na.omit(patron.df)

return.date = as.POSIXlt(patron.df$`Return Date`,format ="%m/%d/%Y",tz="EST" )
patron.df = cbind(patron.df, return.date)
length.days = difftime(patron.df$return.date,patron.df$date.time, units = "days" )
patron.df= cbind(patron.df, length.days)

patron.df = patron.df %>% filter(length.days >= 0)

colnames(patron.df)[which(names(patron.df) == "newdf$`Patron Group`")] <- "Patron Group"

#limit to just CMU-affiliated patrons

CMU.patron.df = filter(patron.df, grepl("Carnegie Mellon", patron.df$`Patron Group`) )
CMU.patron.df = select(CMU.patron.df, "length.days","Patron Group")

pg = as.factor(patron.df$`Patron Group`)
pg = droplevels(pg)

pg.split = split(CMU.patron.df$length.days, f=CMU.patron.df$`Patron Group`)
lapply(pg.split,margin=2,FUN=mean) 
...
days.2018 = df.2018$date.time
days.2018 =weekdays.POSIXt(days.2018)
num.days2018 = as.data.frame(table(days.2018))

days.2018 <- ordered(days.2018, levels=c("Monday", "Tuesday", "Wednesday", "Thursday", 
"Friday", "Saturday", "Sunday"))

df2018 = as.data.frame(table(days.2018))

df2018$Freq <- ifelse(df2018$days.2018 == "Monday", df2018$Freq/53,df2018$Freq/52)

colnames(df2018)[which(names(df2018) == "Freq")] <- "Average Number of Loans"
df2018
```
...
cat("The two most dominant frequencies are:",max(s$freq),"and", max(s$freq[s$freq != max(s$freq)]))

```
```
Frequency is the number of cycles per unit time. If two most dominiant
frequency is 182.5 and 181.4861, that means the top two frequencies are
roughly 182 cycles during 2018 and 181 cycles during 2018 
(top number of loans from day to day in 2018).
...
#f(NA, date.format ="%m/%d/%Y")
#Error in tryCatch({ : ERROR: bad date format
...
difference between means (true difference in means 
is not equal to 0).
```
...
cat("There are",sum(is.na(newdf$`LC Classification Top Line...2`)),
    "NAs in the column `LC Classification Top Line...2` ","\n"  )
cat("There are",sum(is.na(newdf$`Return Date`)), 
    "NAs in the column `Return Date` ","\n" )

cat("There are",sum(is.na(newdf$`Patron Group`)),
    "NAs in the column `Patron Group` ","\n" )

cat("There are",sum(is.na(newdf$`Begin Publication Date`)),
    "NAs in the column `Begin Publication Date` ","\n" )
...
cat("There are",sum(is.na(data_new1$Publisher)),
    "empty strings in the column `Publisher` ","\n" )
...
ggplot(top.publish, aes(x=publisher.df, y =Freq, fill =publisher.df )) + 
  geom_bar(stat="identity") + xlab(" top eight publishers") +  ylab("total items published") + theme(axis.text.x = element_text(size = 4))   
...
p = ggplot(num.sub.df, aes(x=sapply.out, y =Freq)) + geom_point() + 
  xlab("number of subjects") +  ylab("total items")
...
#numbers
location.number =regexpr("[0-9]+[[//.]]*[0-9]*", all.caps)

num.classify = regmatches(all.caps, location.number)

num.classify = gsub("[^0-9\\.]","",num.classify)

y =as.numeric(num.classify)



hist(y, xlim=c(0,12000), breaks=1000, xlab ="library classification code number",main ="distribution of library classification code number" )
