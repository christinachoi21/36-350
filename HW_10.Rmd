...
my.pattern.birth = "[0-9]+[[:punct:]][0-9]+[[:punct:]][0-9][0-9]"
occurance.birth =regexpr(my.pattern.birth,sprint.lines)
with.regexpr.birth = regmatches(sprint.lines, occurance.birth)

my.pattern.raceday ="[0-9]+[[:punct:]][0-9]+[[:punct:]][0-9][0-9][0-9][0-9]"
occurance.raceday =regexpr(my.pattern.raceday,sprint.lines)
with.regexpr.race = regmatches(sprint.lines, occurance.raceday)


birthday.format = as.Date(sub("(..)$","19\\1",with.regexpr.birth ),format="%d.%m.%Y")

raceday.format = as.Date(with.regexpr.race,format="%d.%m.%Y")


df <- data.frame(birthday = birthday.format, raceday = raceday.format, difference = raceday.format - birthday.format)

library(ggplot2)

ggplot(df, aes(x=difference)) + geom_histogram(bins=9) + labs(x = "Sprinter Age (Days)") 
...
day.of.yr = as.numeric(format(as.Date("2019/04/01",format="%Y/%m/%d"), "%j"))
loan.ts = ts(loan.tab.data,start=c(2019, day.of.yr),frequency=365)
plot(loan.ts)
...
```{r linewidth=80}
s = spectrum(table(loan.ts),log="no")
w = which.max(s$spec)
f = s$freq[w]
max(s$freq) / f 
...
Frequency by definition, it is the number of cycles per unit time. This particular number is the number of cycles per day. 
...
f = function(n)
{
  ...
  return(cat("standard deviation at n=",n,"is", sd(mean.out),"\n"))
...
```{r linewidth=80}
data <- read.csv("http://www.stat.cmu.edu/~mfarag/350/SN_m_tot_V2.0.csv",header=FALSE, sep= ";")

fourth.ts = ts(data$V4, start=c(1749,1), frequency=2)

plot(fourth.ts, xlab='Year', ylab= 'Number of Sunspots')

d = decompose(fourth.ts)
random = d$random
random = random[is.na(random)==FALSE]  
s = spectrum(random,log="no", xlim = c(.45,.5))
...
acf.output = acf(data, plot=FALSE)
