...
```
It appears that the observed result was obtained by alternating the values in x (1 and 2 in this case) that gets multiplied with y:
...
matrix = matrix(rep(c(0,0,0,1),5),ncol=5)

matrix

vector = c(16,12,8,4,20)

finalmatrix = matrix*vector
...
```
```
The predefined attribute is dim (dimensions), which it gives as 2 and 3.
...
index=which.max(as.vector(table(v)))

as.integer(names(table(v)[index]))
...
z = setdiff(v,u) >= 1 & setdiff(v,u)<= 50
setdiff(v,u)[z]

z1 = setdiff(v,u) >= 51 & setdiff(v,u)<= 100
setdiff(v,u)[z1]
